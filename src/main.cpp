    ...
    while (!Done) {
        ...
        while (!MancalaBoard.GameOver()) {
            ...
            while (!MancalaBoard.Move(MancalaBoard.PlayerTurn(), PitNumber - 1)) {
                std::cout << "Invalid move player " << (MancalaBoard.PlayerTurn() + 1)
                          << " please select another pit> ";
                std::cin >> PitNumber;
            }
        }
        std::cout << "Game over, ";
        if (MancalaBoard.PlayerScore(0) < MancalaBoard.PlayerScore(1)) {
            std::cout << "player 2 wins " << MancalaBoard.PlayerScore(1) << " to " << MancalaBoard.PlayerScore(0)
                      << std::endl;
        } else if (MancalaBoard.PlayerScore(0) > MancalaBoard.PlayerScore(1)) {
            std::cout << "player 1 wins " << MancalaBoard.PlayerScore(0) << " to " << MancalaBoard.PlayerScore(1)
                      << std::endl;
        } else {
            std::cout << "tie game " << MancalaBoard.PlayerScore(0) << " to " << MancalaBoard.PlayerScore(1)
                      << std::endl;
        }
