...
struct CPath::SImplementation {
    ...
    std::string current_path = CPath::GetCWD();
    std::string home = CPath::GetHome();;
...
CPath::CPath(const CPath &path) : DImplementation(std::make_unique<SImplementation>()) {
    ...
    DImplementation->current_path = path.DImplementation->current_path;
    DImplementation->home = path.DImplementation->home;
}
...
CPath CPath::AbsolutePath(const CPath &path) {
    std:: string conj = GetHome();
    conj  += path.DImplementation->input_path;
    CPath val(conj);
    return val;
}
